...
export default function PrintersList() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [dataPrinters] = await Promise.all([getPrinters()]);
  ...
  const modalTogglePrinter = (printer) => {
    ...
    setModalTitle(
      printer.status === "ATIVO"
        ...
        : "Ativação de impressora"
    );
    setModalBodytext(
      `Você tem certeza que deseja ${
        printer.status === "ATIVO" ? "desativar" : "ativar"
      } a impressora?`
    );
    setModalOpen(true);
  };
  ...
  async function changePrinterStatus() {
    ...
    if (data.type === "success") {
      ...
      printer.status === "ATIVO"
        ? (printer.status = "DESATIVADO")
        : (printer.status = "ATIVO");
    } else {
      toast.error(
        "Não foi possível alterar status da impressora! Por favor, tente novamente."
      );
    }
    setModalOpen(false);
  }
  ...
  const filteredPrinters = useMemo(() => {
    return printers
      .filter((printer) => {
        ...
        const { codigoLocadora, ip, padrao, numeroSerie } = printer;
        ...
        <div className="printerslist-container">
          <div className="printerslist-header">
            <div className="printerslist-header-title">
              ...
              <h4 data-testid="filter_beign_shown">
            ...
            filteredPrinters.map((printer) => (
              ...
              >
                <div className="printerslist-model">
                  <Link
                    to={`/visualizarimpressora/${encodeURIComponent(
                      btoa(JSON.stringify(printer))
                    )}`}
                    style={{ color: printer.status === "ATIVO" ? "" : "gray" }}
                  >
                ...
                <div
                  className="printerslist-identification"
                  style={{ color: printer.status === "ATIVO" ? "" : "gray" }}
                >
                ...
                <div
                  className="printerslist-location-counter"
                  style={{ color: printer.status === "ATIVO" ? "" : "gray" }}
                >
                ...
                <div
                  className="printerslist-counter-date"
                  style={{ color: printer.status === "ATIVO" ? "" : "gray" }}
                >
                  <h6>
                    Data do último contador:{" "}
                    {extractDate(printer.dataUltimoContador)}
                  </h6>
                </div>
                  ...
                  <div tabIndex="0" className="printerslist-engine-dropdown">
                    <div className="printerslist-printer-dropdown">
                      <Link
                        to="#"
                        tabIndex="0"
                        onClick={() => modalTogglePrinter(printer)}
                      >
                        {printer.status === "ATIVO" ? "Desativar" : "Ativar"}
                      </Link>
                      <Link
                        to={`/editarimpressora/${btoa(
